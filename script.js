...
async function loadDataFromFile(){
  try{
    ...
    if(r.ok){catalogData=migrateData(await r.json());if(canUseStorage)localStorage.setItem(LS_KEY,JSON.stringify(catalogData));return catalogData;}
  }catch(e){}
...
async function saveToFile(){
  try{
    ...
    const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download="index.html";a.click();URL.revokeObjectURL(url);
...
function downloadExcel(){
  ...
  const url=URL.createObjectURL(blob);const a=document.createElement('a');a.href=url;a.download='tempus_kz_catalog.xls';a.click();URL.revokeObjectURL(url);
}
...
// ========== РЕНДЕР ==========
async function render(){const grid=document.getElementById("grid");if(!grid)return;if(!catalogData.length)await loadDataFromFile();const watches=getFilteredWatches(),all=loadWatchesSync();document.body.classList.toggle("authed",isAuthed);const info=document.getElementById("resultsInfo");if(info)info.innerHTML=(currentCategory!=="all"||priceFilterMin!==null||priceFilterMax!==null||searchQuery)?`Найдено: <b>${watches.length}</b> из <b>${all.length}</b>`:"";if(!watches.length){grid.innerHTML='<div class="empty">Ничего не найдено.</div>';return;}let html='';watches.forEach((w,i)=>{const s=stockInfo(w.qty),images=w.images||[],multi=images.length>1;const article=w.article||"",favActive=isFav(article);const sc=images.length?images.map(src=>`<div class="slide"><img src="${src}" alt="" draggable="false"></div>`).join(""):`<div class="placeholder">${placeholderSVG()}</div>`;const dots=multi?`<div class="slider-dots">${images.map((_,k)=>`<button class="slider-dot${k===0?' active':''}" data-k="${k}"></button>`).join("")}</div>`:"";const arrows=multi?`<button class="slider-arrow prev" data-dir="-1">‹</button><button class="slider-arrow next" data-dir="1">›</button>`:"";html+=`<article class="card${favActive?' fav-active':''}" data-article="${escapeHtml(article)}"><button class="fav-btn${favActive?' active':''}" data-fav="${escapeHtml(article)}">${favActive?'❤️':'🤍'}</button><div class="card-actions"><button class="icon-btn edit" data-edit-article="${escapeHtml(article)}">✎</button><button class="icon-btn del" data-del-article="${escapeHtml(article)}">✕</button></div><div class="slider${multi?' has-multi':''}" data-slider="${i}"><div class="slides">${sc}</div>${arrows}${dots}</div><div class="body">${w.category?`<span class="category-badge ${w.category}">${w.category==='women'?'Женские':'Мужские'}</span>`:''}${article?`<div class="article">Артикул: ${escapeHtml(article)}</div>`:''}${w.name?`<p class="name">${escapeHtml(w.name)}</p>`:''}<p class="desc">${escapeHtml(w.desc)}</p><div class="price-wrap"><div class="price">${fmtPrice(w.price)}</div><div class="stock ${s.cls}">${s.text}</div></div></div></article>`;});grid.innerHTML=html;initSliders();initCardEvents();updateFavCount();}
