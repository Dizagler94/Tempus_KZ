// ========== ОЧИСТКА КЕША ==========
(function() {
  const KEY = 'mdt_cache_v10';
  if (localStorage.getItem(KEY) === 'true') return;
  console.log('🧹 v10 — очистка старых данных');
  try {
    const favs = localStorage.getItem('mdt_favorites_v1');
    const gh = localStorage.getItem('mdt_github_v10');
    localStorage.clear();
    if (favs) localStorage.setItem('mdt_favorites_v1', favs);
    if (gh) localStorage.setItem('mdt_github_v10', gh);
...
async function loadDataFromFile() {
  try {
    ...
    if (r.ok) {
      ...
      console.log('📦 data.json:', catalogData.length);
      return catalogData;
    }
  } catch (e) {}
...
async function saveToGithub() {
  const s = getGhSettings();
  if (!s?.token?.trim()) { alert('Сначала настройте GitHub в админ-панели'); return; }
...
function updateFooter(){
  ...
  if(!f)return;
  f.innerHTML=isAuthed?`© 2026 TEMPUS KZ · <a href="#" onclick="toggleAdmin()" style="color:#d4af37;text-decoration:none;">⚙️ Админ-панель</a>`:`© 2026 TEMPUS KZ · Оффлайн-каталог`;
}
...
function updateAuthUI(){
  ...
  if(!area)return;
  if(isAuthed){
    area.innerHTML=`<button class="btn btn-fav" id="favBtnAuthed">❤️ Избранное<span class="fav-count">${favorites.length}</span></button><button class="btn btn-gold" id="addBtn">+ Добавить</button><button class="btn" id="logoutBtn">Выйти</button>`;
  ...
  updateFooter();
  // Скрываем админ-панель при выходе
  if(!isAuthed){
...
function adminAddRow(){
  catalogData.push({article:'TK-'+new Date().getFullYear()+'-'+String(catalogData.length+1).padStart(3,'0'),name:'',desc:'',category:'men',price:0,qty:0,images:[]});
...
function renderAdminTable(){
  ...
  tbody.innerHTML=catalogData.map((w,i)=>`
    <tr>
      ...
      <td><select data-ai="${i}" data-af="category"><option value="men" ${w.category==='men'?'selected':''}>Муж</option><option value="women" ${w.category==='women'?'selected':''}>Жен</option></select></td>
      ...
      <td><input value="${(w.images||[]).join(', ')}" data-ai="${i}" data-af="imagesStr" style="width:150px;"></td>
      <td style="white-space:nowrap;">
        <button class="icon-btn" onclick="adminSaveRow(${i})" title="Сохранить" style="display:inline-flex;">💾</button>
        <button class="icon-btn danger" onclick="adminDeleteRow(${i})" title="Удалить" style="display:inline-flex;">🗑️</button>
      </td>
    </tr>
  `).join("");
}

function adminSaveRow(i){
  document.querySelectorAll(`[data-ai="${i}"]`).forEach(el=>{
    const field=el.getAttribute("data-af");
    let val=el.value;
    if(field==='price'||field==='qty')val=Number(val)||0;
    else if(field==='imagesStr'){catalogData[i].images=val.split(',').map(s=>s.trim()).filter(s=>s);return;}
    catalogData[i][field]=val;
  });
  saveWatches(catalogData);
  render();
  adminStatus('✅ Сохранено','success');
}
...
function adminStatus(msg,type){
  ...
  el.style.background=type==='success'?'rgba(39,174,96,0.2)':type==='error'?'rgba(192,57,43,0.2)':'rgba(52,152,219,0.2)';
  el.style.color=type==='success'?'#2ecc71':type==='error'?'#e74c3c':'#3498db';
  el.style.border='1px solid '+(type==='success'?'rgba(39,174,96,0.3)':type==='error'?'rgba(192,57,43,0.3)':'rgba(52,152,219,0.3)');
...
function startAutoRefresh(){
  ...
  autoRefreshInterval=setInterval(async ()=>{
    ...
    render();
    if(document.getElementById("adminPanel").classList.contains("show"))renderAdminTable();
...
// ========== ЗАПУСК ==========
console.log('🚀 TEMPUS KZ v10 — встроенная админ-панель + автообновление');
